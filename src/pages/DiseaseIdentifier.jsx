// src/pages/DiseaseIdentifier.jsx
...
const DiseaseIdentifier = () => {
  ...
  // Simulate file upload progress for display only
  const uploadFile = (file) => {
    return new Promise((resolve) => {
      ...
      }, 200); // Increase or decrease for faster/slower progress simulation
    });
  };
  ...
  const handleFiles = (newFiles) => {
    ...
    // Add an ID + progress property to each file
    ...
    // Add to existing files in state
    setFiles((prev) => [...prev, ...filesWithProgress]);

    // Start fake "upload" progress for each new file
  ...
  // Helper to display file type label
  ...
  // Separate completed vs uploading
    ...
    <div className="w-full">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold mb-2">
          Upload your leaf images to analyze using our
          <span className="text-green-600"> AI-based disease detection</span>
        </h1>
      </div>
      ...
      <AnimatePresence>
        ...
        {files.length > 0 && (
          ...
          >
            {/* 
              Use <Link> and pass the files in "state", 
              so that DiseaseAnalyze.jsx can retrieve them. 
            */}
            <Link
              to="/disease-analyze"
              state={{ files }}
